...
const CartDrawer = () => {
  ...
  // Fetch or sync available stock for products currently in cart
  useEffect(() => {
    ...
    const fetchStocks = async () => {
      const uniqueProductIds = [
        ...new Set(
          items
            .map((it) => it.product?.id || it.productId || it.id)
            .filter(Boolean),
        ),
      ];
      ...
      try {
        const responses = await Promise.allSettled(
          uniqueProductIds.map((id) =>
            api.get(`/store/${id}`, { signal: controller.signal }),
          ),
        );
        ...
        responses.forEach((res) => {
          if (res.status === "fulfilled" && res.value?.data) {
            ...
            const totalStock = Number.parseInt(productData.stock, 10) || 0;
            ...
            items
              .filter(
                (it) =>
                  String(it.product?.id || it.productId || it.id) ===
                  String(prodId),
              )
              .forEach((it) => {
                const itemSize = parseSize(it.size || it.product?.size);
                const key = `${prodId}-${itemSize || "nosize"}`;

                if (sizes.length > 0 && itemSize) {
                  const match = sizes.find(
                    (s) => parseSize(s.size) === itemSize,
                  );
                  newMap[key] = match
                    ? Number.parseInt(match.stock, 10) || 0
                    : 0;
                } else {
                  newMap[key] = totalStock;
                }
              });
          }
        });
        ...
        if (error.name !== "CanceledError" && error.message !== "canceled") {
          console.error("Failed to fetch product stock in cart:", error);
        }
      }
    };
  ...
  const getItemAvailableQuantity = (item, displaySize) => {
    const prodId = item.product?.id || item.productId || item.id;
    const itemKey = `${prodId}-${displaySize || "nosize"}`;

    if (typeof stockMap[itemKey] === "number") {
      return stockMap[itemKey];
    }
    ...
    const sizes = item.sizes || item.product?.sizes;
    if (Array.isArray(sizes) && sizes.length > 0 && displaySize) {
      const matchingSize = sizes.find(
        (s) => parseSize(typeof s === "object" ? s.size : s) === displaySize,
      );
      if (matchingSize && typeof matchingSize.stock === "number") {
        return matchingSize.stock;
      }
    }
    if (typeof item.product?.availableQuantity === "number") {
      return item.product.availableQuantity;
    }
  ...
  const handleQuantityChange = async (
    ...
    // Instantly update UI
    dispatch(
      updateItemQuantityLocal({ productId, size, quantity: newQuantity }),
    );
        ...
        <div className="flex-grow overflow-y-auto p-6 flex flex-col gap-4">
          {loading ? (
            ...
            items.map((item) => {
              ...
              const productName =
                item.product?.name || item.product?.productName || "Jersey";
              const productImage =
                item.product?.imageUrl ||
                item.product?.productImageUrl ||
                "https://via.placeholder.com/150";

              const availableQuantity = getItemAvailableQuantity(
                item,
                displaySize,
              );
              ...
              return (
                <div
                  key={`${productId}-${displaySize || "nosize"}`}
                  className="flex gap-4 bg-surface-base p-4 rounded-large border border-white/5"
                >
                  ...
                  <div className="flex flex-col justify-between flex-grow">
                    <div className="flex justify-between items-start">
                      <div>
                        <h3 className="kinetic-heading text-sm text-white line-clamp-2 pr-4">
                          {productName}
                        </h3>
                        ...
                        {typeof availableQuantity === "number" &&
                        availableQuantity <= 0 ? (
                          <div className="text-[10px] text-red-400 font-medium font-inter mt-1">
                            Out of stock
                          </div>
                        ) : isIncreaseDisabled ? (
                          <div className="text-[10px] text-amber-400/90 font-medium font-inter mt-1">
                            Max stock reached ({availableQuantity} available)
                          </div>
                        ) : null}
                      </div>

                      <button
                        onClick={() => handleRemoveItem(productId, displaySize)}
                    ...
                    <div className="flex items-center justify-between mt-2">
                      ...
                      <div className="flex items-center gap-2 bg-surface-high rounded-full border border-white/5 px-1 py-1">
                        ...
                        <button
                          ...
                          title={
                            isIncreaseDisabled
                              ? `Maximum available quantity reached (${availableQuantity})`
                              : "Increase quantity"
                          }
